...
| ![](screenshots/screenshot-main.png) | ![](screenshots/screenshot-main-compact.png) |
|---|---|
| полный режим | компактный режим |


> Лицензия: GNU GPL v3 (`LICENSE.md`) — бесплатное opensource-приложение.
> © 2026 Роман Ермаков
>
> Подробное техническое описание: [DETAILS.md](DETAILS.md)
...
1. Опубликуйте релиз (см. "Ручная сборка" выше) в постоянную папку, например `C:\Quince\`.
   ...
   >[!WARNING]
   > Если файлы на сервер копирует одна учётная запись (например, вы сами, зайдя как администратор или через сетевую шару `\\сервер\c$\...`), а служба/приложение потом запускается под **другой** (например, отдельный пользователь для этой задачи) — унаследованные права NTFS на `config\`/`log\` могут не дать этой другой учётной записи писать в свои же файлы. Симптом в логе: `Не удалось сохранить settings.yaml ... под учётной записью ДОМЕН\пользователь` / `UnauthorizedAccessException: Access to the path '...' is denied` — при этом `icacls` на папке может выглядеть нормально (нужную учётную запись стоит проверить `whoami /groups` на UAC-фильтрацию группы Administrators, а саму папку — явной выдачей прав ниже). Исправляется явной выдачей прав после копирования:
   > ```
   > icacls C:\Quince /grant "ДОМЕН\пользователь:(OI)(CI)M" /T
   > ```
   > Надёжнее всего вообще не полагаться на права конкретного интерактивного пользователя — установить и запускать как службу Windows (см. ниже): по умолчанию она работает под `LocalSystem`, у которого и так есть полный доступ к локальным файлам.
...
>[!TIP] Обновление службы на удалённом компьютере скриптом
> ```powershell
> .\release\Update-Quince.ps1 -InstallPath \\SERVERNAME\c$\Quince -SourcePath .\release\1.00.999
> ```
> Предполагается что новая версия приложения собрана в папку `.\release\1.00.999`, а обновляемая версия находится на `\\SERVERNAME\c$\Quince`
...
- `config\`
  ...
  Путь к папке каналов настраивается переменной окружения `ConfigDir` (по умолчанию `"config"`) — единственная настройка, которая не может жить в `settings.yaml`, поскольку именно она указывает, где этот файл искать.
- `log\` — файлы журнала, по одному на день.
  Путь к папке журналов настраивается переменной окружения `LogDir` (по умолчанию `"log"`).
...
- `config.demo\` - образцы конфигураций. Перед первым запуском скопируйте в `config\` и отредактируйте под себя:
  ...
  >[!INFO]
  > Без этого шага приложение всё равно запустится — `config\` создастся пустым автоматически, каналов записи не будет, авторизация будет выключена.
...
Для плавного отображения индикаторов в браузере аудиопоток буферизируется на 7 секунд. Увеличьте значение если индикаторы "примерзают". Воспроизведение прослушки идет с учетом этого буфера.
...
Включает возможность получать из AoIP-сети Livewire мультикаст на выбранный сетевой интерфейс.

  > [!TIP]
  > Если подключение к Livewire не нужно, выберите сетевой интерфейс = Нет. В этом случае, а также если выбранный сетевой интерфейс отключён этой кнопкой до конца сессии, источника "Livewire" в карточке канала не будет.

### Метаданные
Логгер старается получить из интернет-потоков доступные метаданные - ICY из Icecast, интегрированный в HLS metadata.json или встроенные теги. Полученные метаданные сохраняются в .CSV в подпапку `meta\` внутри папки записи канала, имена файлов соответствуют дате.
...
- для звуковой карты - в WAV PCM с той же частотой дискретизации и числом каналов, с которыми шёл захват;
- для HLS-потока - в AAC с оригинальным битрейтом (можно указать индекс битрейта 0 / 1 / 2 из мультибитрейтного `playlist.m3u8`);
- для Icecast - в MP3 с оригинальным битрейтом;
- для Livewire - в WAV PCM 48000/16 bit/Stereo.

Калькулятор внизу покажет сколько дискового места будет занято на этот канал при данных параметрах записи.

| ![](screenshots/screenshot-channel-detect.png) |
|---|

При обнаружении тишины (3 секунды звук был тише порога в -60 dBFS) запись останавливается и возобновляется через 1 секунду после устойчивого появления звука выше порога. Есть возможность не останавливать запись при обнаружении тишины: только отображать индикатор и писать в лог.
...
# Группы AD с правами администратора — участники get IsAdmin (полный UI с правами создания, редактирования, удаления).
admin_groups:
  - "CN=RadioAdmins,OU=Groups,DC=corp,DC=local"

# Остальным - только просмотр и прослушка.

# Если access_groups задать непустым, входить смогут только участники этих групп (остальные аутентифицированные
...
- `a.pushkin` (перебор всех настроенных доменов),
- `CORP\a.pushkin` (только домен с `name: CORP`),
- `a.pushkin@corp.local` (только домен с таким UPN-суффиксом, выводится из `base_dn` или задаётся явно полем `upn_suffix`).

**Для авториазции через Active Directory** можно указать группу администраторов (тогда все остальные авторизованные будут простыми пользователями) и отдельно группу пользователей (тогда все остальные не будут иметь доступа вообще).

**При локальной авторизации** пользователю можно добавить или отнять права админа (см. `users.yaml` ниже).

Администраторам доступны создание, редактирование, клонирование, удаление каналов записи; запуск-остановка записи; изменение общих настроек и мониторинг ресурсов. 
...
При необходимости обновить/заменить `ffmpeg.exe` и `ffprobe.exe` просто перезапишите файлы тем же именем в той же папке, рестарт приложения подхватит новую версию.

**`scripts\Find-Replace.ps1`** — рекурсивный поиск-замена литерального текста (не regex) во всех файлах папки. Поддерживает стандартные `-WhatIf`/`-Confirm` для предпросмотра без реальной записи.
```powershell
.\scripts\Find-Replace.ps1 -Find 'C:\RECORDING' -Replace 'D:\RECORDING' -Path C:\Quince\config -Filter 'NRG*.yaml'

# Предпросмотр без изменения файлов
.\scripts\Find-Replace.ps1 -Find 'старое' -Replace 'новое' -Path .\config -WhatIf
```
